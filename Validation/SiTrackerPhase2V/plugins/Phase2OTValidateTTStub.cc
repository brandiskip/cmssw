...
public:
  ...
  std::vector<double> getTPDerivedCoords(
      edm::Ptr<TrackingParticle> my_tp, bool isBarrel, double modMaxZ, double modMinZ, float stub_r) const;
...
std::vector<double> Phase2OTValidateTTStub::getTPDerivedCoords(
    edm::Ptr<TrackingParticle> my_tp, bool isBarrel, double modMaxZ, double modMinZ, float stub_r) const {
  double tp_phi = -99;
  double tp_r = -99;
  double tp_z = -99;
...
void Phase2OTValidateTTStub::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (auto gd = theTrackerGeom_->dets().begin(); gd != theTrackerGeom_->dets().end(); gd++) {
    ...
    for (auto stubIter = stubs.begin(); stubIter != stubs.end(); ++stubIter) {
      ...
      float trackBend =
          -(sensorSpacing * stub_r * bfield_ * c_ * myTP_charge) / (stripPitch * 2.0E2 * myTP_pt * correctionValue);
